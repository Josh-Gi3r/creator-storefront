/**
 * Google Maps API proxy helper
 * 
 * Main function: makeRequest<T>(endpoint, params) - Makes authenticated requests to Google Maps APIs
 * Credentials are read from env vars: MAPS_API_BASE_URL, MAPS_API_KEY. Array parameters use | as separator.
 * 
...
function getMapsConfig(): MapsConfig {
  const baseUrl = process.env.MAPS_API_BASE_URL ?? ENV.llmApiBaseUrl;
  const apiKey = process.env.MAPS_API_KEY ?? ENV.llmApiKey;

  if (!baseUrl || !apiKey) {
    throw new Error(
      "Maps proxy credentials missing: set MAPS_API_BASE_URL and MAPS_API_KEY (or LLM_API_BASE_URL + LLM_API_KEY for a unified proxy)"
    );
  }
